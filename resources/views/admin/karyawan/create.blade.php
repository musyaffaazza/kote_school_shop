...
<div class="space-y-6 max-w-4xl mx-auto">
    ...
    <div class="flex items-center gap-2 text-xs font-bold text-[#8f7664] uppercase tracking-wider text-left">
        ...
        <span>&rsaquo;</span>
        <span class="text-[#21140b]">Tambah Karyawan</span>
    </div>
    ...
    <div class="space-y-1 text-left">
        ...
        <p class="text-xs sm:text-sm text-[#7b6558] font-semibold leading-relaxed">
            Daftarkan staff baru ke dalam sistem Kote School Shop.
        </p>
    </div>

    {{-- Form Card --}}
    <form action="{{ route('admin.karyawan.store') }}" method="POST" enctype="multipart/form-data" class="rounded-3xl border border-[#ede6df] bg-white shadow-sm overflow-hidden flex flex-col">
        @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
                ...
                <div class="md:col-span-4 space-y-4 flex flex-col items-center">
                    ...
                    <div onclick="document.getElementById('foto').click();" class="w-40 h-40 border-2 border-dashed border-[#ede6df] rounded-full bg-[#fdfbf9] flex flex-col items-center justify-center cursor-pointer hover:bg-[#faf7f2]/50 transition-colors p-2 relative group overflow-hidden shadow-inner">
                        <div class="flex flex-col items-center justify-center text-center space-y-2" id="placeholder-foto">
                        ...
                        <img id="preview-foto" src="#" alt="Preview" class="absolute inset-0 w-full h-full object-cover hidden" />
                        
                        {{-- Hover overlay to change image --}}
                        <div class="absolute inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hidden" id="hover-foto">
                ...
                <div class="md:col-span-8 space-y-5 text-left">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                        ...
                        {{-- Password --}}
                        <div class="space-y-2">
                            <label for="password" class="text-xs font-bold text-[#21140b] uppercase tracking-wider">Password <span class="text-red-500">*</span></label>
                            <input type="password" name="password" id="password" placeholder="Min. 6 karakter" class="w-full bg-[#fdfbf9] border border-[#ede6df] text-xs font-semibold rounded-xl px-4 py-3 text-[#21140b] focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#3d2a1f]/10 focus:border-[#3d2a1f] transition-all @error('password') border-red-500 @enderror" required />
                            @error('password')
                                <p class="text-xs text-red-500 font-semibold mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="alamat" id="alamat" rows="4" placeholder="Tuliskan alamat lengkap di sini..." class="w-full bg-[#fdfbf9] border border-[#ede6df] text-xs font-semibold rounded-2xl px-4 py-3 text-[#21140b] focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#3d2a1f]/10 focus:border-[#3d2a1f] transition-all resize-none @error('alamat') border-red-500 @enderror" required>{{ old('alamat') }}</textarea>
                        @error('alamat')
                            <p class="text-xs text-red-500 font-semibold mt-1">{{ $message }}</p>
                        @enderror
                    </div>
