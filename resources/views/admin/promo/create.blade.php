...
<div class="space-y-6 max-w-4xl mx-auto">
    ...
    <nav class="flex items-center gap-2 text-xs font-semibold text-[#8f7664]">
        ...
        <span class="text-[#21140b] font-bold">Tambah Promo</span>
    </nav>

    {{-- Title --}}
    <h1 class="text-3xl font-extrabold text-[#21140b] tracking-tight">Tambah Promo Baru</h1>
    ...
    <form action="{{ route('admin.promo.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="rounded-2xl bg-white border border-[#e0d5ca]/60 shadow-sm p-6 md:p-8 space-y-6">
            ...
            {{-- Two-Column Grid: Left fields + Right fields --}}
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                {{-- Left Column --}}
                <div class="space-y-5">
                    {{-- Nama Promo --}}
                    <div>
                        <label for="nama_promo" class="block text-sm font-bold text-[#21140b] mb-1.5">Nama Promo <span class="text-red-500">*</span></label>
                        <input type="text" name="nama_promo" id="nama_promo" value="{{ old('nama_promo') }}" placeholder="Contoh: Diskon Kemerdekaan" class="w-full bg-[#faf7f2]/40 border border-[#d5ccc3] rounded-xl px-4 py-3 text-sm text-[#21140b] placeholder-[#b09a87] focus:outline-none focus:ring-2 focus:ring-[#3d2a1f]/15 focus:border-[#8b5a2b] transition-all" required />
                        @error('nama_promo') <p class="text-xs text-red-500 mt-1 font-semibold">{{ $message }}</p> @enderror
                    </div>

                    {{-- Jenis Promo --}}
                    <div>
                        <label for="jenis_promo" class="block text-sm font-bold text-[#21140b] mb-1.5">Jenis Promo <span class="text-red-500">*</span></label>
                        <div class="relative">
                            <select name="jenis_promo" id="jenis_promo" class="w-full bg-[#faf7f2]/40 border border-[#d5ccc3] rounded-xl px-4 py-3 text-sm text-[#21140b] focus:outline-none focus:ring-2 focus:ring-[#3d2a1f]/15 focus:border-[#8b5a2b] transition-all appearance-none cursor-pointer pr-10" required>
                                <option value="" disabled {{ old('jenis_promo') ? '' : 'selected' }}>Pilih jenis promo</option>
                                ...
                                <option value="voucher" {{ old('jenis_promo') === 'voucher' ? 'selected' : '' }}>VOUCHER</option>
                            </select>
                    ...
                    {{-- Nilai Promo --}}
                    <div>
                        <label for="nilai_promo" class="block text-sm font-bold text-[#21140b] mb-1.5">Nilai Promo <span class="text-red-500">*</span></label>
                        <div class="flex items-center gap-2">
                            <input type="number" name="nilai_promo" id="nilai_promo" value="{{ old('nilai_promo', 0) }}" min="0" class="flex-1 bg-[#faf7f2]/40 border border-[#d5ccc3] rounded-xl px-4 py-3 text-sm text-[#21140b] focus:outline-none focus:ring-2 focus:ring-[#3d2a1f]/15 focus:border-[#8b5a2b] transition-all" required />
                            <div class="relative shrink-0">
                                <select name="satuan_nilai" id="satuan_nilai" class="bg-[#faf7f2]/40 border border-[#d5ccc3] rounded-xl pl-3 pr-8 py-3 text-sm text-[#21140b] focus:outline-none focus:ring-2 focus:ring-[#3d2a1f]/15 focus:border-[#8b5a2b] transition-all cursor-pointer appearance-none">
                                    <option value="persen" {{ old('satuan_nilai') === 'persen' ? 'selected' : '' }}>%</option>
                                    <option value="rupiah" {{ old('satuan_nilai', 'rupiah') === 'rupiah' ? 'selected' : '' }}>Rp</option>
                                </select>
                    ...
                    {{-- Kode Voucher --}}
                    <div>
                        <label for="kode_voucher" class="block text-sm font-bold text-[#21140b] mb-1.5">Kode Voucher (Opsional)</label>
                        <input type="text" name="kode_voucher" id="kode_voucher" value="{{ old('kode_voucher') }}" placeholder="CONTOH: MERDEKA45" class="w-full bg-[#faf7f2]/40 border border-[#d5ccc3] rounded-xl px-4 py-3 text-sm text-[#21140b] placeholder-[#b09a87] focus:outline-none focus:ring-2 focus:ring-[#3d2a1f]/15 focus:border-[#8b5a2b] transition-all uppercase" />
                        @error('kode_voucher') <p class="text-xs text-red-500 mt-1 font-semibold">{{ $message }}</p> @enderror
                    </div>

                    {{-- Periode Berlaku --}}
                    <div>
                        <label for="periode_mulai" class="block text-sm font-bold text-[#21140b] mb-1.5">Periode Berlaku <span class="text-red-500">*</span></label>
                        <input type="date" name="periode_mulai" id="periode_mulai" value="{{ old('periode_mulai') }}" class="w-full bg-[#faf7f2]/40 border border-[#d5ccc3] rounded-xl px-4 py-3 text-sm text-[#21140b] focus:outline-none focus:ring-2 focus:ring-[#3d2a1f]/15 focus:border-[#8b5a2b] transition-all" />
                        @error('periode_mulai') <p class="text-xs text-red-500 mt-1 font-semibold">{{ $message }}</p> @enderror
                    </div>

                    {{-- Hingga --}}
                    <div>
                        <label for="periode_selesai" class="block text-sm font-bold text-[#21140b] mb-1.5">Hingga</label>
                        <input type="date" name="periode_selesai" id="periode_selesai" value="{{ old('periode_selesai') }}" class="w-full bg-[#faf7f2]/40 border border-[#d5ccc3] rounded-xl px-4 py-3 text-sm text-[#21140b] focus:outline-none focus:ring-2 focus:ring-[#3d2a1f]/15 focus:border-[#8b5a2b] transition-all" />
                ...
                <div class="flex flex-col justify-between space-y-5">
                    ...
                    <div class="flex-1 flex flex-col">
                        ...
                        <textarea name="deskripsi" id="deskripsi" placeholder="Jelaskan detail dan syarat ketentuan promo..." class="w-full bg-[#faf7f2]/40 border border-[#d5ccc3] rounded-xl px-4 py-3 text-sm text-[#21140b] placeholder-[#b09a87] focus:outline-none focus:ring-2 focus:ring-[#3d2a1f]/15 focus:border-[#8b5a2b] transition-all resize-none flex-1 min-h-[220px] md:min-h-[280px]">{{ old('deskripsi') }}</textarea>
                    ...
                    <div class="flex items-center justify-between bg-[#faf7f2] border border-[#e0d5ca]/60 rounded-xl px-5 py-4">
                        <div>
                            ...
                            <p class="text-xs text-[#8f7664]">Aktifkan untuk langsung menampilkan promo</p>
                        </div>
            ...
            {{-- Unggah Gambar --}}
            <div class="space-y-3">
                <label class="block text-sm font-bold text-[#21140b]">Unggah Gambar Promo</label>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div id="file-info-container" class="flex items-center gap-2 border border-[#d5ccc3] rounded-xl px-4 py-2.5 bg-[#faf7f2]/40 w-full overflow-hidden hidden">
                        ...
                        <a href="#" id="file-link" target="_blank" class="text-xs font-bold text-[#8b5a2b] hover:text-[#3d2a1f] underline truncate select-none">
                    ...
                    <span id="no-file-text" class="text-xs text-[#8f7664] font-medium">Belum ada file dipilih</span>
                </div>
