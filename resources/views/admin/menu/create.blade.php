...
<div class="space-y-6 max-w-4xl mx-auto">
    ...
    <div class="flex items-center gap-2 text-xs font-bold text-[#8f7664] uppercase tracking-wider text-left">
        ...
        <span>&rsaquo;</span>
        <span class="text-[#21140b]">Tambah Menu</span>
    </div>
    ...
    {{-- Form Card --}}
    <form action="{{ route('admin.menu.store') }}" method="POST" enctype="multipart/form-data" class="rounded-3xl border border-[#ede6df] bg-white shadow-sm overflow-hidden flex flex-col">
        @csrf
        ...
        <div class="p-6 sm:p-8">
            <div class="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
                ...
                <div class="md:col-span-5 space-y-4">
                    {{-- Input files (Hidden) --}}
                    <input type="file" name="gambar" id="gambar" accept="image/png, image/jpeg, image/jpg, image/webp" class="hidden" onchange="previewImage(this, 'main')" required />
                    ...
                    <div onclick="document.getElementById('gambar').click();" class="w-full aspect-square border-2 border-dashed border-[#ede6df] rounded-[1.5rem] bg-[#fdfbf9] flex flex-col items-center justify-center cursor-pointer hover:bg-[#faf7f2]/50 transition-colors p-4 relative group overflow-hidden shadow-inner">
                        <div class="flex flex-col items-center justify-center text-center space-y-2" id="placeholder-main">
                        ...
                        <img id="preview-main" src="#" alt="Main Preview" class="absolute inset-0 w-full h-full object-cover hidden" />
                        
                        {{-- Hover overlay to change image --}}
                        <div class="absolute inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hidden" id="hover-main">
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        @foreach(['sub_1', 'sub_2', 'sub_3'] as $subKey)
                            <div onclick="document.getElementById('gambar_{{ $subKey }}').click();" class="aspect-square border-2 border-dashed border-[#ede6df] rounded-2xl bg-[#fdfbf9] flex items-center justify-center cursor-pointer hover:bg-[#faf7f2]/50 transition-colors relative overflow-hidden group shadow-inner">
                                <div class="flex flex-col items-center justify-center text-[#b5a49a]" id="placeholder-{{ $subKey }}">
                                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.75">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" />
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 13a3 3 0 11-6 0 3 3 0 016 0z" />
                                    </svg>
                                </div>
                                <img id="preview-{{ $subKey }}" src="#" alt="{{ $subKey }} Preview" class="absolute inset-0 w-full h-full object-cover hidden" />
                                
                                {{-- Hover overlay --}}
                                <div class="absolute inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hidden" id="hover-{{ $subKey }}">
                ...
                <div class="md:col-span-7 space-y-5 text-left">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div class="space-y-2 relative">
                            ...
                            <select name="kategori" id="kategori" class="w-full appearance-none bg-[#fdfbf9] border border-[#ede6df] text-xs font-semibold rounded-xl pl-4 pr-10 py-3.5 text-[#21140b] focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#3d2a1f]/10 focus:border-[#3d2a1f] transition-all cursor-pointer @error('kategori') border-red-500 @enderror" required>
                                ...
                                <option value="Tea" {{ old('kategori') === 'Tea' ? 'selected' : '' }}>Tea</option>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        {{-- Stok Awal --}}
                        <div class="space-y-2">
                            <label for="stok" class="text-xs font-bold text-[#21140b] uppercase tracking-wider">Stok Awal</label>
                            <input type="number" name="stok" id="stok" value="{{ old('stok', 0) }}" placeholder="0" min="0" class="w-full bg-[#fdfbf9] border border-[#ede6df] text-xs font-semibold rounded-xl px-4 py-3.5 text-[#21140b] focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#3d2a1f]/10 focus:border-[#3d2a1f] transition-all @error('stok') border-red-500 @enderror" />
                            @error('stok')
                                <p class="text-xs text-red-500 font-semibold mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="deskripsi" id="deskripsi" rows="6" placeholder="Tuliskan deskripsi menu di sini..." class="w-full bg-[#fdfbf9] border border-[#ede6df] text-xs font-semibold rounded-2xl px-4 py-3.5 text-[#21140b] focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#3d2a1f]/10 focus:border-[#3d2a1f] transition-all resize-none @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi') }}</textarea>
                        @error('deskripsi')
                            <p class="text-xs text-red-500 font-semibold mt-1">{{ $message }}</p>
                        @enderror
                    </div>
